  ...
  content = templatefile("${path.module}/hosts.ini.tpl",
    {
      ...
      connect_public_ip          = try(google_compute_instance.connect[0].network_interface[0].access_config[0].nat_ip)
      connect_private_ip         = try(google_compute_instance.connect[0].network_interface[0].network_ip)
